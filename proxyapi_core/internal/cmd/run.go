...
// openUI in a separate goroutine automatically after server is started.
func openUI(cfg *config.Config) {
	time.Sleep(1500 * time.Millisecond) // Give the server time to start serving Web UI
	url := fmt.Sprintf("http://localhost:%d", cfg.Port)
	if cfg.TLS.Enable {
		url = fmt.Sprintf("https://localhost:%d", cfg.Port)
	}
	log.Infof("Auto-opening Web UI in browser: %s", url)
	if err := util.OpenBrowser(url); err != nil {
		log.Warnf("Could not auto-open browser: %v", err)
	}
}
