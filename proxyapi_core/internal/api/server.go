...
import (
	...
	"github.com/mad-agentic/ProxyAPI.MAD/v6/internal/access"
	managementHandlers "github.com/mad-agentic/ProxyAPI.MAD/v6/internal/api/handlers/management"
	"github.com/mad-agentic/ProxyAPI.MAD/v6/internal/api/middleware"
	"github.com/mad-agentic/ProxyAPI.MAD/v6/internal/api/modules"
	...
	"github.com/mad-agentic/ProxyAPI.MAD/v6/internal/managementasset"
	"github.com/mad-agentic/ProxyAPI.MAD/v6/internal/usage"
	"github.com/mad-agentic/ProxyAPI.MAD/v6/internal/util"
	sdkaccess "github.com/mad-agentic/ProxyAPI.MAD/v6/sdk/access"
	"github.com/mad-agentic/ProxyAPI.MAD/v6/sdk/api/handlers"
	"github.com/mad-agentic/ProxyAPI.MAD/v6/sdk/api/handlers/claude"
	"github.com/mad-agentic/ProxyAPI.MAD/v6/sdk/api/handlers/gemini"
	"github.com/mad-agentic/ProxyAPI.MAD/v6/sdk/api/handlers/openai"
	sdkAuth "github.com/mad-agentic/ProxyAPI.MAD/v6/sdk/auth"
	"github.com/mad-agentic/ProxyAPI.MAD/v6/sdk/cliproxy/auth"
...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	if stats := usage.GetRequestStatistics(); stats != nil {
		if err := stats.ConfigurePersistence(usage.ResolvePersistencePath(cfg)); err != nil {
			log.Warnf("failed to configure usage persistence: %v", err)
		}
	}
	...
	// Serve Static UI directly from API Server (SPA Fallback)
	// We map the root and fallback routes to serve our React/Solid app built in ../frontend/dist
	publicDir := filepath.Join(filepath.Dir(s.currentPath), "frontend", "dist")
	s.engine.Static("/assets", filepath.Join(publicDir, "assets"))
	s.engine.StaticFile("/vite.svg", filepath.Join(publicDir, "vite.svg"))

	s.engine.NoRoute(func(c *gin.Context) {
		// Avoid intercepting API routes that just weren't found
		...
		// Fallback everything else to index.html for SPA router
	...
	hasManagementSecret := cfg.RemoteManagement.SecretKey != "" || envManagementSecret || s.localPassword != "" || !cfg.CommercialMode
...
func (s *Server) setupRoutes() {
	...
	unifiedModels := s.unifiedModelsHandler(openaiHandlers, claudeCodeHandlers)

	// OpenAI compatible API routes
	v1 := s.engine.Group("/v1")
	v1.Use(AuthMiddleware(s.accessManager))
	{
		v1.GET("/models", unifiedModels)
	...
	// Engine conversion routes (alias endpoint proxy)
	// Examples:
	// - /v1/engines/client/models -> /v1/models
	// - /v1/engines/visual/models -> /v1/models (Gemini-oriented alias)
	// - /v1/engines/client/chat/completions -> /v1/chat/completions
	v1Engine := s.engine.Group("/v1/engines/:engine")
	v1Engine.Use(AuthMiddleware(s.accessManager))
	{
		v1Engine.GET("/models", func(c *gin.Context) {
			engine := normalizeEngineAlias(c.Param("engine"))
			switch engine {
			case "gemini":
			...
			case "openai", "claude":
				unifiedModels(c)
			default:
				c.JSON(http.StatusBadRequest, gin.H{"error": "unsupported engine", "engine": c.Param("engine")})
			}
		})

		v1Engine.POST("/chat/completions", func(c *gin.Context) {
			engine := normalizeEngineAlias(c.Param("engine"))
			switch engine {
			case "openai", "claude", "gemini":
				openaiHandlers.ChatCompletions(c)
			default:
				c.JSON(http.StatusBadRequest, gin.H{"error": "unsupported engine", "engine": c.Param("engine")})
			}
		})

		v1Engine.POST("/completions", func(c *gin.Context) {
			engine := normalizeEngineAlias(c.Param("engine"))
			switch engine {
			case "openai", "claude", "gemini":
				openaiHandlers.Completions(c)
			default:
				c.JSON(http.StatusBadRequest, gin.H{"error": "unsupported engine", "engine": c.Param("engine")})
			}
		})

		v1Engine.POST("/messages", func(c *gin.Context) {
			engine := normalizeEngineAlias(c.Param("engine"))
			switch engine {
			case "claude":
				claudeCodeHandlers.ClaudeMessages(c)
			case "openai", "gemini":
				openaiHandlers.ChatCompletions(c)
			default:
				c.JSON(http.StatusBadRequest, gin.H{"error": "unsupported engine", "engine": c.Param("engine")})
		...
		v1Engine.POST("/messages/count_tokens", func(c *gin.Context) {
			engine := normalizeEngineAlias(c.Param("engine"))
			switch engine {
			case "claude":
				claudeCodeHandlers.ClaudeCountTokens(c)
			default:
				c.JSON(http.StatusBadRequest, gin.H{"error": "count_tokens is only available for claude/anthropic engine", "engine": c.Param("engine")})
			}
		})

		v1Engine.POST("/responses", func(c *gin.Context) {
			engine := normalizeEngineAlias(c.Param("engine"))
			switch engine {
			case "openai", "claude", "gemini":
				openaiResponsesHandlers.Responses(c)
			default:
				c.JSON(http.StatusBadRequest, gin.H{"error": "unsupported engine", "engine": c.Param("engine")})
			}
		})

		v1Engine.POST("/responses/compact", func(c *gin.Context) {
			engine := normalizeEngineAlias(c.Param("engine"))
			switch engine {
			case "openai", "claude", "gemini":
				openaiResponsesHandlers.Compact(c)
			default:
				c.JSON(http.StatusBadRequest, gin.H{"error": "unsupported engine", "engine": c.Param("engine")})
			}
		})
	...
	// Root endpoint giờ sẽ chuyển fallback cho web UI
...
func normalizeEngineAlias(engine string) string {
	key := strings.ToLower(strings.TrimSpace(engine))
	switch key {
	case "openai", "codex", "client", "cli", "default":
		return "openai"
	case "anthropic", "claude":
		return "claude"
	case "gemini", "google", "visual", "vision", "vissual", "image":
		return "gemini"
	default:
		return key
	}
...
func (s *Server) UpdateClients(cfg *config.Config) {
	...
	if stats := usage.GetRequestStatistics(); stats != nil {
		if err := stats.ConfigurePersistence(usage.ResolvePersistencePath(cfg)); err != nil {
			log.Warnf("failed to reconfigure usage persistence: %v", err)
		}
	}
	...
	newHasSecret := cfg.RemoteManagement.SecretKey != "" || s.envManagementSecret || s.localPassword != "" || !cfg.CommercialMode
	prevHasSecret := s.managementRoutesEnabled.Load()

	if newHasSecret && !prevHasSecret {
		s.registerManagementRoutes()
		if s.managementRoutesEnabled.CompareAndSwap(false, true) {
			log.Info("management routes enabled after configuration update")
		} else {
			s.managementRoutesEnabled.Store(true)
		}
	} else if !newHasSecret && prevHasSecret {
		if s.managementRoutesEnabled.CompareAndSwap(true, false) {
			log.Info("management routes disabled after configuration update")
		} else {
			s.managementRoutesEnabled.Store(false)
		}
	} else {
		s.managementRoutesEnabled.Store(newHasSecret)
		if newHasSecret {
			s.registerManagementRoutes()
		}
	}
