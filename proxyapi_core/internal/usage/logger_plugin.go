...
import (
	...
	coreusage "github.com/mad-agentic/ProxyAPI.MAD/v6/sdk/cliproxy/usage"
	log "github.com/sirupsen/logrus"
)

var statisticsEnabled atomic.Bool

const usagePersistenceDebounce = 1 * time.Second
...
type persistenceFilePayload struct {
...
// ResolvePersistencePath returns the file used to persist usage snapshots across restarts.
func ResolvePersistencePath(cfg *config.Config) string {
	return filepath.Join(logging.ResolveLogDirectory(cfg), "usage-stats.json")
}

// ConfigurePersistence enables snapshot persistence for the shared usage store.
func (s *RequestStatistics) ConfigurePersistence(path string) error {
	...
	s.persistMu.Lock()
	if cleanPath != "" && s.persistSignal == nil {
		s.persistSignal = make(chan struct{}, 1)
		go s.persistenceLoop()
	}
	previouslyLoaded := s.loadedPersistTo
	s.persistPath = cleanPath
	s.persistMu.Unlock()

	if cleanPath == "" {
		return nil
	}
	if previouslyLoaded == cleanPath || !s.isEmpty() {
		return nil
	}
	if err := s.LoadSnapshotFromFile(cleanPath); err != nil {
		return err
	}

	s.persistMu.Lock()
	s.loadedPersistTo = cleanPath
	s.persistMu.Unlock()
	return nil
}

// LoadSnapshotFromFile merges a previously saved usage snapshot into memory.
func (s *RequestStatistics) LoadSnapshotFromFile(path string) error {
	...
	var payload persistenceFilePayload
	if err := json.Unmarshal(data, &payload); err != nil {
		return fmt.Errorf("parse usage snapshot: %w", err)
	}
	if payload.Version != 0 && payload.Version != 1 {
		return fmt.Errorf("unsupported usage snapshot version: %d", payload.Version)
	}

	result := s.MergeSnapshot(payload.Usage)
	if result.Added > 0 {
		log.Infof("usage statistics restored from %s (%d records)", path, result.Added)
	}
	return nil
}

// SaveSnapshotToFile writes the current usage snapshot atomically to disk.
func (s *RequestStatistics) SaveSnapshotToFile(path string) error {
	if s == nil || strings.TrimSpace(path) == "" {
		return nil
	}
	...
	payload := persistenceFilePayload{
		Version: 1,
	...
	if err := os.Remove(cleanPath); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("remove previous usage snapshot file: %w", err)
	}
...
func (s *RequestStatistics) persistenceLoop() {
	for range s.persistSignal {
		time.Sleep(usagePersistenceDebounce)
		for {
			select {
			...
			default:
				...
				if path != "" {
					if err := s.SaveSnapshotToFile(path); err != nil {
						log.Warnf("failed to persist usage statistics to %s: %v", path, err)
					}
				}
				goto persisted
			}
